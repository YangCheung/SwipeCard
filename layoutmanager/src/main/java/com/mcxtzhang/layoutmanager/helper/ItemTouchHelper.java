...
package com.mcxtzhang.layoutmanager.helper;
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if(action == MotionEvent.ACTION_MOVE) {
                    final float dx = event.getX(index) - mInitialTouchX;
                    final float dy = event.getY(index) - mInitialTouchY;

                    final float xDiff = Math.abs(dx);
                    final float yDiff = Math.abs(dy);
                    if (DEBUG)
                        Log.v(TAG, "Moved x to "
                                   + event.getX()
                                   + ","
                                   + event.getY()
                                   + " diff="
                    ...
                    if (xDiff > mSlop && xDiff * 0.5f > yDiff && dx > 0) {
                        if (DEBUG)
                            Log.v(TAG, "disable drag!");
                        disableSwipe = true;
                    }
                }
                if (DEBUG) {
            ...
            public void run() {
                if (mRecyclerView != null && mRecyclerView.isAttachedToWindow()
                    ...
                    if ((animator == null || !animator.isRunning(null))
                        ...
                        mCallback.onSwiped(ItemTouchHelper.this, anim.mViewHolder, swipeDir);
                    } else {
    ...
    public void removeAnimation(ViewHolder viewHolder) {
        final int recoverAnimSize = mRecoverAnimations.size();
        for (int i = recoverAnimSize - 1; i >= 0; i--) {
            final RecoverAnimation anim = mRecoverAnimations.get(i);
            if (anim.mViewHolder == viewHolder) {
    ...
    private ViewHolder findSwipedView(MotionEvent motionEvent) {
        ...
        MotionEvent fakeEvent = motionEvent;
        if (disableSwipe) {
    ...
    public abstract static class Callback {
         ...
         *                   #END}).
         */
        public abstract void onSwiped(ItemTouchHelper touchHelper, ViewHolder viewHolder, int direction);
        ...
        void onDraw(Canvas c, RecyclerView parent, ViewHolder selected,
            ...
            Log.d("recoverAnimSize", "recoverAnimSize = " + recoverAnimSize);
